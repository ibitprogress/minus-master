...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        for user in orm['auth.User'].objects.all():
            ...
            if profile.name: user.first_name = profile.name[:29]
            else: user.first_name = u''
            if profile.surname: user.last_name = profile.surname[:29] 
            else: user.last_name = u''
            user.save()
    ...
    def backwards(self, orm):
        ...
        for user in orm['auth.User'].objects.all():
            profile,c = orm.UserProfile.objects.get_or_create(user= user)
            profile.name = user.first_name or u''
            profile.surname = user.last_name or u''
            profile.save()
