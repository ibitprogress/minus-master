...
@register.inclusion_tag('djangoratings/top_rated.html')
def top_rated(model):
    """ Represents list of top rated object in the given model """
    if model == 'MinusRecord':
        rated_objects = MinusRecord.objects.filter(rating_score__gt=0)
        top_rated_objects = rated_objects.order_by('-rating_score')[:settings.TOP_RATED_LIMIT]
    return {
