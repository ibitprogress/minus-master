...
def is_friend_of(value, arg):
    """
    Returns True or False whether a user in the context
    is a friend of request.user object
    """
    return bool(Friendship.objects.get(user=value).friends.filter(user=arg).count())

@register.filter
def invitations_to_count(value):
    """Returns count of unaccepted (new) invitations"""
    return value.invitations_to.filter(accepted=False).count()
