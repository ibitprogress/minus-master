...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """
        Based on new minusstore models validation.
        some code copied, because it's one-time-use function
        """
        for author in orm.MinusAuthor.objects.all():

            #clean stopwords on authors
            for w in author.name.split():
                qs = orm.MinusStopWord.objects.filter(word = w.lower)
                if qs:
                    print author.name,
                    stopword = qs[0]
                    ...
                    print " - ", author.name
                    author.save()
                    author = orm.MinusAuthor.objects.get(id = author.id) #tricky reload

            #remove brackets
            cleanedname = author.name.replace('"','')
            if author.name != cleanedname:
                author.name = cleanedname
                author.save()


        for record in orm.MinusRecord.objects.all():
            #clean stopwords in titles
    ...
    models = {
        ...
        'minusstore.minusstopword': {
            'Meta': {'object_name': 'MinusStopWord'},
