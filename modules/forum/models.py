...
class Forum(models.Model):
    ...
    and posts fields are updated by the save() methods of their
...
class Thread(models.Model):
    ...
    sticky = models.BooleanField("Прикріплена?", blank=True, default=False)
    closed = models.BooleanField("Закрита?", blank=True, default=False)
    posts = models.IntegerField("Повідомлення", default=0)
    views = models.IntegerField("Перегляди", default=0)
    latest_post_time = models.DateTimeField("Дата останнього повідомлення", blank=True,
    ...
    def get_thread_author(self):
        posts = self.post_set.all().reverse()
        post = posts[:1].get()
        return post.author
...
class Post(models.Model):
    ...
    """
    thread = models.ForeignKey(Thread, verbose_name="Тема")
    author = models.ForeignKey(User, related_name='forum_post_set',
                              verbose_name="Автор")
    body = models.TextField("Тіло повідомлення")
    body_html = models.TextField("Відрендерене тіло повідомлення",
                                 editable=False)
    time = models.DateTimeField("Час", blank=True, null=True)
    ...
    def get_absolute_url(self):
        # Dirty hack :D 10 is posts count per page
        post_per_page = 10
        post_count = self.thread.posts
        pages_count = int(ceil(float(post_count)/post_per_page))
        return '%s?page=%s#post%s' % (self.thread.get_absolute_url(),
                                      pages_count, self.id)
