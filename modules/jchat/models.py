...
class Room(models.Model):
    ...
    def __unicode__(self):
        return u'Chat for %s %d' % (self.content_type, self.object_id)
...
class Message(models.Model):
    ...
    author = models.ForeignKey(UserProfile, related_name='author', blank=True, null=True)
    ...
    def __unicode__(self):
        ...
        if self.type == 'n':
            ...
            return u'JOIN: %s' % self.author
        elif self.type == 'l':
            return u'LEAVE: %s' % self.author
        elif self.type == 'a':
            return u'ACTION: %s > %s' % (self.author, self.message)
        return self.message
